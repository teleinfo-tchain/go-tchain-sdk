...
//
// // Int64 int64
// // uintx类型序列化（uintx()）或者反序列化(putuintx())
...
// 	if len(data) > 1 && ok {
// 		rest := data[1:]
// 		//fmt.Println(reflect.TypeOf(data), reflect.TypeOf(types), reflect.TypeOf(rest))
// 		//fmt.Printf("types is %v,  rest is %v, restLen, %d typesLen is%d \n ", types, rest, len(rest), len(types))
...
// 	//fmt.Printf("type is %v, values is %v \n", types, values)
// 	var b [][]byte
...
// 	//fmt.Println("res is ", hash.Sum(nil))
// 	return hash.Sum(nil)
// }
//
...
// 	//fmt.Printf("typ %v  matchesLen %v  o %v  value %v \n", typ, matches, len(matches), value)
// 	if len(matches) > 0 {
// 		match := matches[0]
//
...
// 		//fmt.Printf("str is %v, isSting is %v \n", str, isString)
