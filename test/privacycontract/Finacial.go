...
type Finacial struct {
	super *BaseContract
	abi   abi.ABI
	bin   []byte
}

func (base *BaseContract) NewFinacial() *Finacial {
	parseAbi, _ := abi.JSON(strings.NewReader(FinacialAbiJSON))

	finacial := new(Finacial)
	finacial.super = base
	finacial.abi = parseAbi
	finacial.bin, _ = hex.DecodeString(FinacialBin)
	return finacial
}

// 部署合約
func (base *Finacial) FinacialDeploy(signTxParams *BaseTxParams) (string, error) {
...
//發行接口
// 日志：Issue
func (base *Finacial) FinacialIssue(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	...
	// encoding
	inputEncode, err := base.abi.Pack("issue", name, symbol, commitA, rangeproofA, from_pubkey)
	if err != nil {
		return "", err
	}
...
//轉移接口
func (base *Finacial) FinacialTransfer(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	if !isValidHexAddress(contractAddress) {
		return "", errors.New("contractAddress is not valid address")
	}
	var base_blind string = "1"
	...
	inputEncode, err := base.abi.Pack("transfer", inputId, commits, rangeproofs, encryptvalue, fromPubkeys, toadds)
	if err != nil {
		return "", err
	}
...
//查詢代筆名接口
func (base *Finacial) FinacialQueryName(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	if !isValidHexAddress(contractAddress) {
		return "", errors.New("contractAddress is not valid address")
	}
...
//查詢代筆標志接口
func (base *Finacial) FinacialQuerySymbol(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	if !isValidHexAddress(contractAddress) {
		return "", errors.New("contractAddress is not valid address")
	}
