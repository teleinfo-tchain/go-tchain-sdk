...
// BaseContract - The BaseContract Module
...
// NewBaseContract - BaseContract Module constructor to set the default provider
func NewBaseContract(provider providers.ProviderInterface) *BaseContract {
...
*/
func (base *BaseContract) prePareSignTransaction(signTxParams *BaseTxParams, payLoad []byte, contractAddr string) (string, error) {
	bid, privateKey, err := account.Decrypt(signTxParams.KeyFileData, signTxParams.IsSM2, signTxParams.Password)
	if err != nil {
		return "", err
	}

	sender := utils.StringToAddress(bid)
...
func (base *BaseContract) prePareSignDeployTransaction(signTxParams *BaseTxParams, payLoad []byte) (string, error) {
	_, _, err := account.Decrypt(signTxParams.KeyFileData, signTxParams.IsSM2, signTxParams.Password)
	if err != nil {
		return "", err
	}
	...
	signResult, err := account.SignTransaction(signTx, "d4b8727f468ecd8c2a809b99658f243b268ef2b36d3e0e7f9d9bac9fc8fa78e9", false)
	if err != nil {
		return "", err
	}
...
func (base *BaseContract) structToInterface(convert interface{}, values []interface{}) []interface{} {
	elem := reflect.ValueOf(convert)
	for i := 0; i < elem.NumField(); i++ {
		field := elem.Field(i)
		if field.Kind() == reflect.Struct {
			values = append(base.structToInterface(field.Interface(), values))
		} else {
...
*/
func (base *BaseContract) sendRawTransaction(encodedTx string) (string, error) {
	...
	err := base.provider.SendRequest(&pointer, "core_sendRawTransaction", params)
...
func (base *BaseContract) LogDecode(transactionHash string) (*LogData, error) {
	params := make([]string, 1)
	params[0] = transactionHash

	pointer := &dto.RequestResult{}

	err := base.provider.SendRequest(pointer, "core_getTransactionReceipt", params)
