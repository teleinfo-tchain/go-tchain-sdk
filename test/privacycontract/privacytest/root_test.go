...
// 部署合約
func TestRootDeploy(t *testing.T) {
	...
	Root := connection.PrivacyContract.NewRoot()

	txHash, err := Root.RootDeploy(txParams)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
// 日志：Issue
func TestRootIssue(t *testing.T) {
	var connection = bif.NewBif(providers.NewHTTPProvider(resources.IP52+":"+strconv.FormatUint(resources.Port, 10), 10, false))
	chainId, err := connection.Core.GetChainId()
	if err != nil {
		t.Log(err)
	...
	manager := connection.PrivacyContract.NewRoot()

	contractAddress := resources.TestContractAddress

	txHash, err := manager.RootIssue(txParams, contractAddress)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
// 轉移接口
func TestRootTransfer(t *testing.T) {
	var connection = bif.NewBif(providers.NewHTTPProvider(resources.IP52+":"+strconv.FormatUint(resources.Port, 10), 10, false))
	chainId, err := connection.Core.GetChainId()
	if err != nil {
		t.Log(err)
	...
	txHash, err := manager.RootTransfer(txParams, contractAddress)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
// 查詢代筆名接口
func TestRootQueryname(t *testing.T) {
	var connection = bif.NewBif(providers.NewHTTPProvider(resources.IP52+":"+strconv.FormatUint(resources.Port, 10), 10, false))
	chainId, err := connection.Core.GetChainId()
	if err != nil {
		t.Log(err)
	...
	contractAddress := resources.TestContractAddress
	txHash, err := manager.RootQueryName(txParams, contractAddress)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
// 查詢代筆標志接口
func TestRootQuerysymbol(t *testing.T) {
	var connection = bif.NewBif(providers.NewHTTPProvider(resources.IP52+":"+strconv.FormatUint(resources.Port, 10), 10, false))
	chainId, err := connection.Core.GetChainId()
	if err != nil {
		t.Log(err)
	...
	txHash, err := manager.RootQuerySymbol(txParams, contractAddress)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
