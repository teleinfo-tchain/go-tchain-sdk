...
func (base *BaseContract) NewNft() *Nft {
	parseAbi, _ := abi.JSON(strings.NewReader(NftAbiJSON))

	Nft := new(Nft)
	Nft.super = base
	Nft.abi = parseAbi
	Nft.bin, _ = hex.DecodeString(NftBin)
	return Nft
}

// 部署合約
func (base *Nft) NftDeploy(signTxParams *BaseTxParams) (string, error) {
...
//發行接口
// 日志：Issue
func (base *Nft) NftIssue(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	...
	// encoding
	inputEncode, err := base.abi.Pack("issueCard", name, symbol, from_pubkey, commitA, rangeproofA)
	if err != nil {
		return "", err
	}
...
//轉移接口
func (base *Nft) NftTransfer(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	if !isValidHexAddress(contractAddress) {
		return "", errors.New("contractAddress is not valid address")
	}
	var base_blind string = "1"
	...
	inputEncode, err := base.abi.Pack("transferCard", tokenId, inputId, commits, fromPubkeys, rangeproofs, toadds, encryptvalue)
	if err != nil {
		return "", err
	}
...
//查詢代筆名接口
func (base *Nft) NftQueryName(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	if !isValidHexAddress(contractAddress) {
		return "", errors.New("contractAddress is not valid address")
	}
...
//查詢代筆標志接口
func (base *Nft) NftQuerySymbol(signTxParams *BaseTxParams, contractAddress string) (string, error) {
	if !isValidHexAddress(contractAddress) {
		return "", errors.New("contractAddress is not valid address")
	}
