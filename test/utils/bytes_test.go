...
func (s *BytesSuite) TestLeftPadBytes(c *checker.C) {
	...
	res1 := utils.LeftPadBytes(val1, 8)
	res2 := utils.LeftPadBytes(val1, 2)
...
func (s *BytesSuite) TestRightPadBytes(c *checker.C) {
	...
	resstd := utils.RightPadBytes(val, 8)
	resshrt := utils.RightPadBytes(val, 2)
...
func TestFromHex(t *testing.T) {
	input := "0x01"
	expected := []byte{1}
	result := utils.FromHex(input)
	if !bytes.Equal(expected, result) {
		t.Errorf("Expected %x got %x", expected, result)
	}
}

func TestByteIsHex(t *testing.T) {
	tests := []struct {
		input string
		ok    bool
	}{
	...
	for _, test := range tests {
		if ok := utils.IsHex(test.input); ok != test.ok {
