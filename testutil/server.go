...
func NewTestServer(t *testing.T, cb ServerConfigCallback) *TestServer {
	...
	//dir, err := ioutil.TempDir("D:\\Go\\src\\github.com\\umbracle\\go-web3\\contract", "geth-")
	//if err != nil {
	//	t.Fatalf("err: %s", err)
	//}
	...
	// add ipcpath
	args = append(args, "--ipcpath", filepath.Join(dir, "gbif.ipc"))
...
func (t *TestServer) DeployContract(c *Contract) (*compiler.Artifact, web3.Address) {
	...
	solcContract, err := c.Compile(demoPath)
	if err != nil {
		panic(err)
	}
